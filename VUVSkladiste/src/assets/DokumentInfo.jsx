...
function DokumentInfo() {
    ...
    const handleOpenLokacijaModal = (artDokId) => {
        const artDokIdNum = typeof artDokId === 'number' ? artDokId : parseInt(artDokId, 10);
        const postojecaLokacija = lokacijeArtikala.find(l => parseInt(l.ART_DOK_ID, 10) === artDokIdNum) || null;

        setOdabraniArtDokId(artDokIdNum);
    ...
    const handleSaveLokacija = async () => {
        ...
        const body = {
            // API očekuje originalne nazive iz baze (loK_ID, arT_DOK_ID ...)
            loK_ID: lokId,
            arT_DOK_ID: artDokId,
            red: redValue,
            stupac: stupacValue
        ...
        if (trenutnaLokacijaArtikla?.LOK_ART_ID) {
            body.loK_ART_ID = parseInt(trenutnaLokacijaArtikla.LOK_ART_ID, 10);
        }
        ...
        try {
            ...
            const poruka = err?.response?.data?.message || err?.response?.data?.title || err?.response?.data || err.message;
        ...
        <div className="container mt-4">
            ...
            {artikli.length > 0 ? (
                <Table striped bordered hover>
                    ...
                    <tbody>
                        {artikli.map((a, i) => {
                            const artDokIdNum = typeof a.id === 'number' ? a.id : parseInt(a.id, 10);
                            const lokacijaZapisa = lokacijeArtikala.find(l => parseInt(l.ART_DOK_ID, 10) === artDokIdNum) || null;
                                ...
                                {isPrimka && (
                                    <td>
                                        <Button
                                            variant="warning"
                                            size="sm"
                                            onClick={() => handleOpenLokacijaModal(artDokIdNum)}
                                        >
            ...
            <Modal show={showLokacijaModal} onHide={handleCloseLokacijaModal}>
                ...
                <Modal.Footer>
                    ...
                    <Button variant="primary" onClick={handleSaveLokacija}>
                        {trenutnaLokacijaArtikla ? "Ažuriraj" : "Dodaj"}
                    </Button>
                </Modal.Footer>
            </Modal>
